...
namespace SwitchHelper
{
    ...
    public static class About
    {
        ...
        /// </summary>
        private static APP_INFO AppInfo
        {
            get
            {
                if (_appInfo.AppFullPath == null)
                {
                    ...
                    //get current project binary, either .DLL or EXE
                    var currentAssembly = Assembly.GetExecutingAssembly();
                    //only returns object if parent Assembly exists.
                    var callerAssemblies = new StackTrace().GetFrames()
                                .Select(x => x.GetMethod().ReflectedType.Assembly).Distinct()
                                .Where(x => x.GetReferencedAssemblies().Any(y => y.FullName == currentAssembly.FullName));

                    if (callerAssemblies != null && currentAssembly.Location.EndsWith(".dll"))
                        currentAssembly = callerAssemblies.Last();  //change DLL Assembly to Parent caller's Assembly.
                    ...
                    //gather all network interface card's information
